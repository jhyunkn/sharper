...
// Library tab: smaller search text, one-line domains, Show all / Show saved, and category tag instead of action count.
...
h = h.replace(/\.filters button,\.saved-toggle\{[^}]*\}/, ".filters button,.saved-toggle{font-family:var(--mono);font-size:9px;text-transform:uppercase;letter-spacing:.15em;color:var(--warm)}.filters button{font-size:7.5px;letter-spacing:.08em;text-align:center;white-space:nowrap}");
h = h.replace(/\.saved-toggle\{[^}]*\}/, ".saved-toggle{display:inline-flex;gap:8px;margin-top:20px;margin-right:22px}");
h = h.replace('</style>', ".library-note{margin-top:24px;max-width:300px;font-family:var(--serif);font-style:italic;font-size:16px;line-height:1.5;color:var(--warm)}</style>");
h = h.replace("['leadership','Leadership','△']", "['leadership','Lead','△']");
h = h.replace("['communication','Communication','( )']", "['communication','Speak','( )']");
h = h.replace("<button id=\"savedToggle\" class=\"saved-toggle\">♡ Show saved</button><div id=\"paths\"", "<div><button id=\"showAll\" class=\"saved-toggle\">Show all</button><button id=\"savedToggle\" class=\"saved-toggle\">♡ Show saved</button></div><p class=\"library-note\">An accumulation of everyday specimen examination — the ideas you have inspected, saved, and returned to.</p><div id=\"paths\"");
...
h = h.replace("$('savedToggle').innerHTML=state.savedOnly?'♥ Saved only':'♡ Show saved';", "$('savedToggle').innerHTML=state.savedOnly?'♥ Show saved':'♡ Show saved';");
