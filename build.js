...
h = h.replace(/\.lib-head\{[^}]*\}/, ".lib-head{display:flex;gap:14px;align-items:center;margin:0;padding:0;border:0;background:transparent;backdrop-filter:none}");
...
h = h.replace(/<div class='reader-section'><div class='kicker ink'>Dive deeper<\/div>\$\{p\.read\.map\(x=>'<p>'\+x\+'<\/p>'\)\.join\(''\)\}<\/div>/, "<div class='reader-section'><div class='kicker ink'>Dive deeper</div>${p.read.map(x=>'<p>'+x+'</p>').join('')}<p>Carry this as a small test: notice where the idea appears in a real conversation, decision, or moment of resistance today.</p></div>");
