...
h = h.replace(/\.filters\{[^}]*\}/, ".filters{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-top:24px;border-top:1px solid var(--ruleSoft);border-bottom:1px solid var(--ruleSoft);padding:12px 0}");
h = h.replace(/\.filters button,\.saved-toggle\{[^}]*\}/, ".filters button,.saved-toggle{font-family:var(--mono);font-size:10px;text-transform:uppercase;letter-spacing:.14em;color:var(--warm)}.filters button{font-size:10px;letter-spacing:.14em;text-align:center;white-space:nowrap}");
h = h.replace(/\.saved-toggle\{[^}]*\}/, ".saved-toggle{display:inline-flex;align-items:center;justify-content:center;gap:8px;margin:0;padding:10px 0;font-size:10px!important;letter-spacing:.14em!important}");
h = h.replace('</style>', ".library-note{margin-top:24px;max-width:300px;font-family:var(--serif);font-style:italic;font-size:16px;line-height:1.5;color:var(--warm)}.filter-toggle{display:grid;grid-template-columns:1fr 1fr;border:1px solid var(--ruleSoft);border-radius:999px;overflow:hidden;margin-top:20px}.filter-toggle .saved-toggle{border-radius:0}.filter-toggle .saved-toggle.on{background:rgba(26,23,20,.08);color:var(--ink)}.library .filters button,.library .saved-toggle{font-family:var(--mono)!important;font-size:10px!important;letter-spacing:.14em!important;text-transform:uppercase!important;line-height:1.2!important}</style>");
