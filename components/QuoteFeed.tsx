...
function DomainPill({ category, color }: { category: string; color: string }) {
  return (
    <span
      className="inline-flex w-fit items-center rounded-full border px-3 py-1 text-[9px] tracking-[0.28em] uppercase"
...
function QuoteCard({ quote, isActive, archetypeColor, onToggleSave }: {
  ...
  return (
    <div
      className="relative h-full w-full overflow-hidden rounded-[2rem] border border-[#29261f]"
      style={{
        background:
          'radial-gradient(ellipse 80% 60% at 80% 42%, rgba(201,169,110,0.08), transparent 58%), linear-gradient(180deg, rgba(26,24,19,0.92), rgba(18,17,13,0.96))',
      ...
      <AnimatePresence mode="wait">
        {!contextOpen ? (
          ...
          >
            ...
            <blockquote className={`mt-10 font-serif text-[#ede8df] ${quoteSize(quote.text)}`}>
              {quote.text}
            </blockquote>
          ...
          >
            <div className="px-8 pt-12 pb-[calc(8rem+env(safe-area-inset-bottom))]">
              <button
                ...
                className="mb-10 flex items-center gap-2 text-[9px] tracking-[0.32em] uppercase"
              ...
              <div className="mt-10 space-y-10">
                {quote.historicalContext && (
                  <section className="space-y-3">
                    <DomainPill category="context" color={color} />
                    <p className="text-sm leading-[1.9] text-[#b2a997]">{quote.historicalContext}</p>
                  </section>
                )}
                {quote.meaning && (
                  <section className="space-y-3">
                    <p className="text-[9px] tracking-[0.42em] uppercase" style={{ color: `${color}cc` }}>Meaning</p>
                    <p className="text-sm leading-[1.9] text-[#b2a997]">{quote.meaning}</p>
                  </section>
                )}
                {quote.whyItMatters && (
                  <section className="space-y-3">
                    <p className="text-[9px] tracking-[0.42em] uppercase" style={{ color: `${color}cc` }}>Why it matters</p>
                    <p className="text-sm leading-[1.9] text-[#b2a997]">{quote.whyItMatters}</p>
                  </section>
                )}
              </div>
              ...
              <button
                onClick={() => setContextOpen(false)}
                className="mt-14 w-full rounded-full border border-[#29261f] bg-[#1a1813]/80 px-5 py-4 text-[10px] tracking-[0.32em] uppercase text-[#b2a997]"
              >
                Close card
              </button>
            </div>
...
export default function QuoteFeed() {
  ...
  useEffect(() => {
    ...
    fetchCards(profile.archetypeId, profile.viewedQuoteIds).then((loaded) => {
      setCards(loaded);
      setLoading(false);
    });
  }, [router]);
  ...
  if (loading) {
    return (
      <div className="fixed inset-0 flex items-center justify-center bg-[#12110d]">
        <motion.p animate={{ opacity: [0.2, 0.6, 0.2] }} transition={{ duration: 2.2, repeat: Infinity }} className="text-[9px] tracking-[0.4em] text-[#746d60] uppercase">
          Loading
        </motion.p>
      </div>
    );
  ...
  return (
    <div className="fixed inset-0 overflow-hidden bg-[#12110d]">
      ...
      <div className="relative z-10 px-6 pt-14">
        <p className="text-center font-serif text-2xl tracking-[0.3em] text-[#ede8df]">SHARPER</p>
      </div>

      <div
        className="absolute inset-x-0 top-24 bottom-[calc(5rem+env(safe-area-inset-bottom))] flex snap-x snap-mandatory gap-3 overflow-x-auto px-5"
      ...
      >
        {cards.map((card, index) => (
          <div
            key={card.id}
            className="h-full w-[calc(100dvw-40px)] shrink-0 snap-center"
            onTransitionEnd={() => markViewed(card.id)}
          >
            <QuoteCard
              quote={card}
