...
const DOMAIN_LABEL: Record<string, string> = {
  philosophy: 'Philosophy', psychology: 'Psychology', art: 'Art', architecture: 'Architecture',
  literature: 'Literature', poetry: 'Poetry', science: 'Science', music: 'Music', business: 'Business',
...
async function loadSavedCards(ids: string[]): Promise<Quote[]> {
  ...
  return ids.map((id) => localQuotes.find((q) => q.id === id)).filter(Boolean) as Quote[];
}

function ContextDrawer({ quote, archetypeColor, onClose }: { quote: Quote; archetypeColor: string; onClose: () => void }) {
  ...
  return (
    ...
    >
      <div className="px-8 pt-14 pb-[calc(8rem+env(safe-area-inset-bottom))]">
        <button
          onClick={onClose}
        ...
        <button
          onClick={onClose}
          className="mt-14 w-full border border-[#29261f] bg-[#12110d]/72 px-5 py-4 text-[10px] tracking-[0.32em] uppercase text-[#b2a997] backdrop-blur-sm transition-colors duration-200 active:bg-[#1a1813]"
        >
          Close card
        </button>
      </div>
  ...
  return (
    <div className="fixed inset-0 overflow-hidden bg-[#080808]">
      <main
        ...
        style={{ WebkitOverflowScrolling: 'touch', overscrollBehaviorY: 'contain', scrollbarWidth: 'none' } as React.CSSProperties}
      >
        <div className="px-6 pt-14 pb-8">
          ...
          {savedQuotes.length > 0 && <p className="text-xs text-[#444] mt-1">{savedQuotes.length} thought{savedQuotes.length !== 1 ? 's' : ''}</p>}
        </div>

        {loading ? (
          <div className="flex items-center justify-center pt-16"><p className="text-xs text-[#333]">Loading…</p></div>
        ) : savedQuotes.length === 0 ? (
          <div className="flex flex-col items-center justify-center px-8 pt-16 text-center gap-4">
            <p className="text-4xl opacity-20">☽</p>
            <p className="text-sm text-[#333] leading-relaxed">Nothing saved yet.<br />Bookmark thoughts that move you.</p>
          </div>
        ) : (
          <div className="px-6 space-y-3 pb-8">
            {savedQuotes.map((quote, i) => {
              ...
              return (
                ...
                >
                  <p className="text-[9px] tracking-[0.3em] uppercase" style={{ color: color + '80' }}>{DOMAIN_LABEL[quote.category] ?? quote.category}</p>
                  <blockquote className="font-serif text-base leading-relaxed text-[#d0cbc2]">&ldquo;{quote.text}&rdquo;</blockquote>
                  <div className="flex items-end justify-between">
                    ...
                    <div className="flex items-center gap-3">
                      {hasContext && <div className="flex items-center gap-[4px]">{[0, 0.4, 0.8].map((delay, di) => <motion.span key={di} className="block w-[3px] h-[3px] rounded-full" style={{ background: color }} animate={{ opacity: [0.15, 0.45, 0.15] }} transition={{ duration: 2.4, repeat: Infinity, delay, ease: 'easeInOut' }} />)}</div>}
                      <button onClick={(e) => { e.stopPropagation(); handleUnsave(quote.id); }} className="opacity-0 group-hover:opacity-100 transition-opacity duration-200 p-1 text-[#333] hover:text-[#888]" aria-label="Remove"><BookmarkX size={14} /></button>
                    </div>
                  </div>
                </motion.div>
              );
      ...
      <AnimatePresence>
        {expandedQuote && <ContextDrawer quote={expandedQuote} archetypeColor={archetypeColor} onClose={() => setExpandedQuote(null)} />}
      </AnimatePresence>
    </div>
  );
}
